...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        # Log additional tensor,加载额外的张量
        if not self.is_tb_set:
            with self.model.graph.as_default():
                #tf.summary.scalar用来显示标量信息
                tf.summary.scalar('value_target', tf.reduce_mean(self.model.value_target))#tf.reduce_mean 函数用于计算张量tensor沿着指定的数轴（tensor的某一维度）上的的平均值，主要用作降维或者计算tensor（图像）的平均值。
                self.model.summary = tf.summary.merge_all()#merge_all 可以将所有summary全部保存到磁盘，以便tensorboard显示。如果没有特殊要求，一般用这一句就可一显示训练时的各种信息了
            self.is_tb_set = True
...
LEARN_FUNC_DICT = {
    'a2c': lambda e: A2C(policy="MlpPolicy", learning_rate=1e-3, n_steps=1, gamma=0.7, env=e, seed=0,tensorboard_log="./a2c_tensorboard/").learn(total_timesteps=10000, callback=TensorboardCallback()),
...
def test_identity(model_name):
    ...
    mean_reward, std_reward=evaluate_policy(model, env, n_eval_episodes=20, reward_threshold=None,return_episode_rewards=False)
    print('目前已经完成了评估过程')
    obs = env.reset()
